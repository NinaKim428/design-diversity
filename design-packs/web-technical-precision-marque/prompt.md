...
- 섹션 수직 리듬: 일정한 `96px`(balanced — 불규칙 리듬 금지).
...
## 컴포넌트
- **헤더/내비:** 높이 64px, 배경 `#16181C` + 하단 `1px solid #2A2D33`. 좌측 워드마크(Plex Sans 600), 중앙 메뉴(Plex Mono 0.78rem 대문자), 우측 CTA 보더 버튼. 스크롤 시 헤어라인만 또렷해짐.
...
### 5. 모델 갤러리
풀폭 섹션, 12열을 2열 그리드(각 6열 span)로 모델 이미지 카드 4개. 각 카드 이미지(radius 2px, 채도 -8%) + 하단 캡션 바(`#2A2D33`, Plex Mono 모델명 + 등폭 가격 수치). 카드 4모서리 L자 크롭마크. 호버 시 헤어라인 또렷 + 가격 숫자 밝아짐(이미지 줌 없음).
