...
## 레이아웃 / 그리드
- 콘텐츠 텍스트 폭: `680px` 중앙 정렬. 사진 섹션은 풀뷰포트(100vw × 100vh).
...
## 컴포넌트
- **헤더/내비:** 높이 56px, 투명 또는 흰 배경. 좌측 워드마크(Inter 500), 중앙/우측 메뉴(Inter 400 0.82rem). 보더·그림자 없음. 사진 섹션 위에서는 텍스트 색이 사진 명도에 따라 흰/검 전환.
- **풀뷰포트 섹션:** 100vh 사진 + 중앙 정렬 헤드라인 + 서브카피 + 버튼 페어. 카드·배지 없음.
- **가격 카드:** `#F4F4F4` 면, 보더 없음, radius 4px. 중앙 정렬 — 옵션명 + 가격(Inter 500) + 사양 리스트(불릿 없이 줄바꿈) + 버튼 페어.
- **버튼 페어:** 솔리드 + 외곽선 버튼이 항상 한 쌍, 가로 또는 세로로 중앙 정렬.
